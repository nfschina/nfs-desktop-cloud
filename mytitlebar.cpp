...
void MyTitleBar::initControl()
{
    ...
    m_pTitleContent->installEventFilter(this);
    ...
    m_pButtonQuit->setFixedSize(QSize(48, 24));
    ...
    m_pButtonQuit->setObjectName("ButtonQuit");
 ...
 //   m_pButtonClose->setIcon(QIcon(QPixmap(":/new/index/close")));
    m_pButtonQuit->setToolTip(tr("退出"));
    ...
    mylayout->addWidget(m_pButtonQuit);
    ...
    mylayout->setContentsMargins(0, 4, 4, 0);
...
{
    if(obj == m_pTitleContent)
    {
      if (event->type() == QEvent::MouseMove) {
          //QMoveEvent *movEvent = static_cast<QMoveEvent*>(event);
...
void MyTitleBar::initConnections()
{
    ...
    connect(m_pButtonQuit, SIGNAL(clicked()), this, SLOT(onButtonCloseClicked()));
}
...
void MyTitleBar::setTitleContent(QString titleContent, int titleFontSize)
{
    ...
    m_pTitleContent->setAlignment(Qt::AlignBottom|Qt::AlignRight);
...
void MyTitleBar::mousePressEvent(QMouseEvent *event)
{
    if (m_buttonType == MIN_MAX_BUTTON)
    {
        ...
        {
            m_isPressed = true;
            qDebug()<<"max is visible and pressed";
